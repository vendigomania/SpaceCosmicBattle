...
namespace MatchThreeMachine
{
	public static class TileDataMatrixUtility
	{
		...
		public static MatchData FindBestMatch(TileData[,] tiles)
		{
			var bestMatch = default(MatchData);
			...
			for (var y = 0; y < tiles.GetLength(1); y++)
			{
				for (var x = 0; x < tiles.GetLength(0); x++)
				{
					...
					var match = new MatchData(tile, h, v);
					...
					else if (match.Count > bestMatch.Count) bestMatch = match;
				}
			}
		...
		public static List<MatchData> FindAllMatches(TileData[,] tiles)
		{
			...
			for (var y = 0; y < tiles.GetLength(1); y++)
			{
				for (var x = 0; x < tiles.GetLength(0); x++)
				{
					...
					if (match.Count > -1) matches.Add(match);
				}
			}
		...
		public static MoveInfo FindMove(TileData[,] tiles)
		{
			...
			for (var y = 0; y < height; y++)
			{
				for (var x = 0; x < width; x++)
				{
					for (byte d = 0; d <= 3; d++)
					{
						...
						if (FindBestMatch(tilesCopy) != null) return new MoveInfo(x, y, x2, y2);
		...
		public static MoveInfo FindBestMove(TileData[,] tiles)
		{
			var tilesCopy = (TileData[,])tiles.Clone();
			...
			for (var y = 0; y < height; y++)
			{
				for (var x = 0; x < width; x++)
				{
					for (byte d = 0; d <= 3; d++)
					{
						...
						if (match != null && match.Count > bestScore)
						{
							bestMove = new MoveInfo(x, y, x2, y2);

							bestScore = match.Count;
						}
